    ...
    Instructions.Nullary i ->
	  ...
	  case value of
        ...
        Instructions.Print  -> do
          let addr = (smem ! (rtp-1)) -- gets the address of the string
          --
          let dlist = elems dmem -- converting it into a list

          let len = dlist !! (fromIntegral(addr-3) )
          let dlist2 = dropsome ((addr-3)-len) dlist -- droping elements that are above the string 
           
          let newdlist3 = takesome len dlist2  --  
          -- converting it into chars
         
          let dlist4 = map fromIntegral newdlist3
          let dcharlist = reverse (map chr dlist4)
          tell $ [show $ dcharlist]
          
         
          put $ machine { rpc = rpc + 1, rtp = rtp - 1 }
          run
        Instructions.LoadT  -> do
          -- memory mapped control and status registers implemented here
           -- tell $ [show $ (smem ! (rtp-1))]
          let addr = (smem ! (rtp-1)) -- gets the address 
           
          put $ machine { rpc = rpc + 1,
                                 smem = (smem // [(rtp-1, (dmem ! (addr-3)))]) }
          
          run

          
        Instructions.StoT  -> do
           
          let  val= (smem ! (rtp-1)) -- gets the address 
          let addr = (smem ! (rtp-2)) 

          put $ machine { rpc = rpc + 1,rtp = rtp - 2,
                                 dmem = (dmem // [(addr-3,  val)]) }
          run
